#!/bin/bash
# Generate artifacts/weights/loftr_outdoor_state_dict.safetensors using the
# Kornia LoFTR pretrained weights.
#
# This replicates the loftr-export Dockerfile stage locally.
# Run from the loftr-rs workspace root:
#
#   ./scripts/generate_loftr_state_dict.sh
#
# Prerequisites: python3, internet access (downloads ~45 MB kornia weights on first run).
# The venv is cached at .venv-loftr so subsequent runs are fast.
...
info "Exporting LoFTR outdoor state dict ..."
